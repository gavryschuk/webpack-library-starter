...
function () {
  ...
  _createClass(AI, [{
    ...
    value: function _generateRegExp(x, y) {
      ...
      var checkTopElement = function checkTopElement(x, y, i, lettersCounter) {
        if ((vertical ? y - i : x - i) > 0) {
          if (lettersCounter && _this._isAString(_this._board[vertical ? y - i - 1 : y][vertical ? x : x - i - 1])) {
        ...
        if ((vertical ? y : x) + i < this._boardSize && !isBottomBlocked) {
          ...
          if (checkIfBlocker(element)) {
            if (this._isAString(element) && !lettersCounter) {
              ...
              if (i > 1 && this._isAString(element)) wordArray[vertical ? y + i - 1 : x + i - 1] = null; // check the above on top element isn't blocker

              checkTopElement(x, y, i, lettersCounter);
            }
          } else {
            wordArray[vertical ? y + i : x + i] = '[a-z]'; // check the above on top element isn't blocker
